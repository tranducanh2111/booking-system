...
async function encrypt(text) {
    return new Promise((resolve, reject) => {
        if (typeof text !== 'string') {
            return reject(
                new TypeError('The "text" argument must be of type string.')
            );
        }
        ...
        if (keyBuffer.length !== 32) {
            return reject(
                new Error(
                    'Invalid key length. Key must be 32 bytes for AES-256.'
                )
            );
        }
...
async function decrypt(hash) {
    return new Promise((resolve, reject) => {
        if (!hash || !hash.iv || !hash.content) {
            return reject(
                new TypeError(
                    'Invalid hash object. Must contain iv and content.'
                )
            );
        }
